...
**The second page is a guest's heap, and a declaration does not show it.** `wasm-ld` puts a block's statics and its shadow stack in the declared minimum, and nothing else: `dlmalloc` on `wasm32-unknown-unknown` takes every byte it hands out from `memory.grow`, so the *first* `eio_alloc` a block ever serves leaves the page it declared. Reading one page off a golden block's memory section and reserving one page is therefore the same class of mistake as the 17-page shadow stack below, in the opposite direction: a number that looked like a measurement and was a declaration. At a one-page bound `counter` fails `eio_configure` with `ERR_LIMIT` before a single signal is routed.

`crates/leaf/tests/memory_growth.rs` is where the two pages come from, and it re-takes the measurement on every `just ci` rather than quoting it: it runs §9's suite 1 on WAMR's interpreter at every page bound from one upward and prints the smallest each scenario holds at. Every scenario driving an SDK-built golden block needs exactly **two**; every hand-written `.wat` fixture needs one; nothing in the suite needs three. It asserts both directions — nothing over the reserve, and something *at* it — so the number cannot quietly become slack.
...
1. **A module whose declared maximum exceeds the reserve is refused by the generator**, in the same call and the same walk as its declared minimum (§6.4.5, `eio_manifest::validate_against`). It has stated an appetite this target cannot meet. Capping it at the engine instead is not a gentler answer, it is a *wrong* one: ABI §4.1 forbids granting an instance less than it declared at either end, because the instance then fails at whatever allocation first crosses a line the guest was never told about, at whatever moment the traffic reaches it. That an engine will do exactly this if asked is measured, not supposed — `crates/leaf/tests/memory_growth.rs` hands WAMR a module declaring four pages and watches it silently receive two, with no diagnostic anywhere. A build refusal names the block, names both numbers, and costs a build.
