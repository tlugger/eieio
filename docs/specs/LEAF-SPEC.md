...
### 3.3 Which WAMR, and who chooses it

**WAMR 2.4.3**, and it is chosen in exactly one line: `wamrx-sys = "=0.3.0"` in the workspace `Cargo.toml`. That crate vendors `wasm-micro-runtime`'s C core and builds it, so its version *is* the engine's — 0.3.0's tree reads MAJOR 2, MINOR 4, PATCH 3 in `core/version.h`. Two members take it, `crates/wamr-host` as a normal dependency and `crates/conformance` as a dev one, both by inheritance and neither with a `features` key.

**Why this is worth a spec paragraph when an ordinary dependency is not.** §3.1 makes the engine the *only* place ABI §4.3's accepted feature set is enforced: WAMR selects proposals at CMake time, nothing in this repository's own code states which are on, and §6.1.1 records that a `.aot`'s `feature_flags` word is checked against the proposals the runtime was **compiled** with. So the vendored tree decides what a leaf accepts, and before the pin a `cargo update` could have moved that with no edit and no spec change anywhere here (`eieio-x7g.2.28`).

**The pin is deliberate; the version is still the dependency's to pick.** `=0.3.0` fixes *which* `wamrx-sys`, not which WAMR — that mapping is `wamrx-sys`'s, is not published as an API, and is discoverable only by reading `core/version.h` in the vendored tree, which is how the number above was obtained. A `wamrx-sys` bump is therefore an **engine change wearing a dependency's version number**, and the rule that follows is: bumping that line means re-reading `core/version.h` and updating this paragraph in the same commit, exactly as CLAUDE.md requires of any spec-and-code pair.

**What the pin buys is less than it looks, and saying so is the point.** The interpreter's accepted set is not merely asserted here, it is *measured*, and the measurement runs in `just ci`:

- `crates/conformance/tests/wamr.rs`'s `wamr_refuses_every_proposal_outside_the_accepted_set` drives all nine of ABI §4.3's refused proposals through `wasm_runtime_load`. A bump that turned one **on** fails it.
- The same file's ABI §4.3 instruction table drives every instruction of the six accepted proposals' portable subset. A bump that turned one of the six **off** fails that.
- The ABI §13 scenario suite runs on the same linked engine from both sides — the reference harness's fourth host and `crates/leaf`'s own `wamr` binding — so a behavioural move shows up twice.

So a feature-set move would be **caught with or without the pin**. What the pin adds is attribution and timing: a red conformance test the morning after an unrelated `cargo update` is a mystery to be bisected, and an edit to a `=` requirement is a decision with a reviewer attached. And it covers the half no test here can reach: **nothing in this repository has ever built a `.aot`**, so neither the `feature_flags` word nor `AOT_CURRENT_VERSION` is measured by anything (§6.1.1), and for those the pin is the only control that exists.

**Two residues, recorded rather than answered.** First, **wasm3 is not pinned the same way.** §3.1 makes the same statement about it — what it accepts is what it was compiled with — but the crate that vendors wasm3's C source is `wasm3x-sys`, one level below the `wasm3x` its two callers name, so an exact requirement on `wasm3x` would not reach the engine. `Cargo.lock` is what pins it today. Second, the `iwasm` **binary** installed on a developer machine is a different artifact from the linked engine and has been a different version (2.4.5 against 2.4.3); §6.1 named the binary's number until §6.1.1 corrected it, and nothing here should read a version off a locally installed tool again.
...
|Reserve|Size|Why|
|---|---|---|
|Guest linear memory|2 × 64 KiB|**One** 64 KiB page per instance, and two instances. Both numbers are corrections — the page count in one direction, the instance count in both — and the paragraphs below the table are the measurements that forced each. This is the dominant per-instance cost by an order of magnitude.|
...
|Subtotal|192 KiB||
|**Floor**|**192 KiB**|The subtotal exactly, which it has not been before: the two instances the row above names are the most this floor carries, at 2 × 64 + 2 × 8 + 48 = 192 KiB with nothing left over. The 8 KiB stack is the only picked number in the table, and the margin that used to be spelled as a rounding now lives inside it.|
...
**So a v1 leaf image carries two block instances, and that is the headline rather than a footnote.** It said two, then one, and says two again, which is the argument for stating it as an arithmetic rather than as a number: `crates/leaf`'s `V1_MAX_INSTANCES` is `(floor − working set) / (pages × 64 KiB + exec stack)`, evaluated from this table's own rows, and `crates/leaf-gen/tests/parity.rs` divides the floor out independently rather than asserting the answer. Every input to it has been wrong at least once — the page reserve read 17 while SDK §5.2 had no link default, then two while `dlmalloc` declined the linker's remainder; the execution-stack row was 8 MiB in the binding beneath it — and a hard-coded `2` would have survived all of that unchanged and been wrong after each.

**The lever that was taken was the third one.** When the reserve read two pages, two instances wanted 2 × 128 + 2 × 8 + 48 = 320 KiB against a part with 313 KiB, and the levers were listed here as `dram2_seg`'s ~66 KiB, a part with more SRAM, or *a guest heap smaller than `dlmalloc`'s, which is a block-side question the SDK has never been asked*. It has now been asked (SDK §4.1), and the answer was not a smaller heap: it was that `dlmalloc` at its default granularity was declining ≈ 38 KiB the linker had already put inside the page every block declares. Configuring the granularity recovered the page at no cost to any tier, so neither of the two hardware levers was spent — **`dram2_seg` is still deliberately excluded and the v1 target is unchanged**. That matters beyond the arithmetic: the two hardware levers are each spent once, and a budget defect that had been read as a hardware shortfall would have spent one of them for nothing.

**Three instances is not offered, and the reason is which number a build fails against.** 3 × 64 + 3 × 8 + 48 = 264 KiB, which is under the 313 KiB of DRAM and over this floor — so the *part* could hold three and the *floor* does not promise them. The floor is the conservative number by design (it is what a link fails against, and §4.1's 32 KiB native stack sits outside it, leaving ≈ 17 KiB for the image's `.data`/`.bss` and WAMR's runtime globals at three instances against ≈ 89 KiB at two). Those statics are on §11's bring-up list precisely because nothing has measured them; raising the count on the strength of an unmeasured remainder is the mistake this section has now made twice in the other direction.

**A service that will not fit is refused at firmware build time, naming the instances.** This is the same MUST, the same class of refusal and the same place as the per-instance page budget above, for the same reason: nothing is loaded on a leaf (§6.3), so the admission a daemon would make at load time falls where the graph is baked. `crates/leaf-gen` refuses a service file with more block instances than the floor carries, listing them — a count is not actionable and the names are, since what a deployer does next is decide which instance moves to another node. It runs after both SERVICE §7 stages, so a service file that is *also* broken is diagnosed for being broken first; and the message names the two levers that are left, neither of which is a rebuild. There is deliberately no runtime check to go with it: an image that discovers on its third `wasm_runtime_create_exec_env` that it has no heap fails on a device with no log stream to say why (§4.6).
...
**The pin above is half enforced now, and the enforced half is this section's own worked example.** The engine a leaf links is still not chosen by any code in this repository — it is `wamrx-sys`'s vendored tree, WAMR **2.4.3** (`core/version.h`) — but *which* `wamrx-sys` is now a deliberate `=0.3.0` in the workspace `Cargo.toml` rather than a caret requirement `cargo update` could move, and §3.3 records the choice, what it buys and what it does not (`eieio-x7g.2.28`). The `iwasm` on the machine this section was written on is **2.4.5**, and it is where an earlier draft's version number came from; a locally installed binary is a different artifact from the linked engine and nothing here should read a version off one again. What stays unenforced is the *other* end of the pair: the first `wamrc` built here will come from a tree nothing in this repository names, and whether it agrees with 2.4.3 on `AOT_CURRENT_VERSION` is unmeasured because no `wamrc` has been built (`eieio-7d8.21`). Pinning one tree for both is the concrete form §11's toolchain-pinning row takes, and it is not something a spec can assert into being.
...
- **The manifest may not survive the AOT compile, so the cross-check is a build-host act either way.** §3.1 already requires `eio_manifest::validate` at firmware build time "where a refusal costs a build rather than a field failure", and that check reads the module's import section and its `eio:manifest` custom section (ABI §4.3, §11) — neither of which a `.aot` is guaranteed to carry. **This remains read from WAMR's documentation and unmeasured** (`eieio-x7g.2.20`), and it is blocked rather than merely undone: measuring it needs a `wamrc`, which needs the LLVM build `eieio-7d8.21` records six blockers against, so no artifact has been compiled here to look inside. The *alignment* half of that same bead was measurable without one and has been measured — §6.4.2 carries the result — and the two are recorded apart for that reason: what a loader requires of a buffer is readable in the loader's source, and what a compiler emits is not readable anywhere but its output. It cannot make the case *for* loading in any event: a device that cannot re-derive a manifest from the artifact it loaded has no way to check that it is the artifact its graph describes.
...
- **It costs less code on the tier least able to pay.** Loading needs a region reader, an index format and a parser for it, an integrity check and the version check above — all in the image, all `no_std`. Linking needs none of them: `include_bytes!` puts the artifact in `.rodata`, which on the v1 target (§6.2) is memory-mapped flash, so the `&'static [u8]` the baked graph carries is already a pointer into flash and no RAM is spent holding it. Whether the engine then copies the text into executable RAM is the engine's question and is identical under both designs, so it is not a discriminator between them.
  ...
  **Two properties of that `&'static [u8]` are the engine's requirements rather than this section's choice, and only one of them is settled.** §6.4.2 settles the first: the buffer's base must be 4-byte aligned, measured, which is why the baked graph reaches an artifact through `include_module!` and not `include_bytes!`. The second is **unsettled and recorded here rather than answered**: WAMR's `wasm_runtime_load` documents that *"the byte buffer must be writable since runtime may change its content for footprint and performance purpose"*, and exempts only AOT XIP modules — *"the runtime doesn't make modifications to the buffer"* (with an exception its own `doc/xip.md` records). A `.rodata` artifact in memory-mapped flash is not writable. So a leaf that links its artifacts either runs its AOT modules in XIP mode or copies each into RAM before loading, and the first is the only one of those that keeps this section's "no RAM is spent holding it". `crates/wamr-host` takes the copy today because it is a host build and RAM is free there; a leaf cannot assume that. **This does not reverse §6.3** — a loaded-from-flash artifact would need the same writable buffer, so it is not a discriminator between the two designs either — but it is a constraint on §6.2's target and on the AOT mode the pipeline compiles for, and it belongs to §11's pipeline item.
...
- **`include_bytes!` alone is not enough, and the requirement is 4 — measured.** It yields an align-1 array, and WAMR reads a module buffer through direct casts, while §6.2's target gives no unaligned-access guarantee. `crates/leaf` therefore provides `eio_leaf::include_module!`, a `#[repr(C)]` struct whose first field is a zero-length array contributing alignment and no bytes, and **a generator MUST emit that rather than a bare `include_bytes!`**. The alignment is `eio_leaf::graph::MODULE_ALIGN`, and both it and the requirement behind it were **read from the engines' own vendored source** (`eieio-x7g.2.20`), replacing the documentation-derived value an earlier draft of this bullet carried:

  - **WAMR, `.aot`: 4.** Normative in WAMR's public header — `core/iwasm/include/wasm_export.h` on `wasm_runtime_load`: *"If it is AOT binary data, it must be 4-byte aligned."* `core/iwasm/aot/aot_loader.c` agrees and, more usefully, **bounds** it: every header field is read through `TEMPLATE_READ`, which advances the cursor with `align_ptr(p, sizeof(type))` and then loads through a bare `*(type *)p`, and no call site in that file asks for more than `sizeof(uint32)`. The one field that would need 8 is read as two 4-byte loads on purpose — `GET_U64_FROM_ADDR` copies `addr[0]` and `addr[1]` into a union — so **8 is deliberately never required**.
  - **WAMR, `.wasm`: 4.** `core/iwasm/interpreter/wasm_loader.c` parses the body bytewise through LEB decoders, but its `read_uint32` is `TEMPLATE_READ_VALUE`, a bare `*(uint32 *)` cast with *no* alignment step at all. It is used exactly twice, on the magic number and the version word at offsets 0 and 4, so both loads inherit the base's alignment directly.
  - **wasm3, `.wasm`: 1.** `source/m3_core.c`'s `Read_u32`, `Read_u64` and `Read_f64` each go through `memcpy`. wasm3 imposes nothing.

  So **16 was excessive rather than wrong** — four times the requirement, at a cost of at most fifteen bytes of `.rodata` per artifact, which is why it was never a bug. The macro is now aligned to exactly `MODULE_ALIGN`, and a `const` assertion inside it holds the wrapper's alignment against that constant, so an edit that narrowed it fails the build rather than faulting on a flashed image.

  **The measurement also says why the guard has to be structural.** Under-aligning an `.aot` is not merely a misaligned load: `TEMPLATE_READ`'s `align_ptr` works on the *absolute* address, so a base that is not a multiple of 4 advances the cursor to a different file offset than `wamrc` wrote and the parse desynchronises. And every one of those loads *succeeds* unaligned on this repository's dev hosts, which permit unaligned access — so no host test can fail on an under-aligned buffer, and the check cannot be a runtime one on the tier that would fault.

- **A host binding's own copy is subject to the same 4, and `Vec<u8>` does not supply it.** WAMR's header also requires the buffer be *writable* and *retained* until unload, so `crates/wamr-host` owns a copy rather than borrowing the caller's slice. That copy was a `Vec<u8>`, which requests alignment 1 and satisfied WAMR only by whatever the global allocator happened to return; it is now an allocation of `u32` words, so the guarantee comes from the allocator's contract instead of its habits. That requirement is the **engine binding's**, not the baked graph's, which is why `eio_wamr_host::MODULE_ALIGN` states it separately from `eio_leaf::graph::MODULE_ALIGN` — `crates/leaf` has to state the linked-in artifact's alignment with the `wamr` feature off.
...
- **Firmware build pipeline mechanics** — toolchain pinning, AOT artifact caching, reproducibility, and how `cargo eio aot` and `eio-leaf-gen` (§6.4.5) are invoked from a Designer deploy. It also owns where a build gets the two things §6.4.5 requires it to state and cannot invent: the node id between builds (§6.4.3) and the bus configuration. **And it owns the residue §6.1.1 hands it** (eieio-x7g.2.18): the WAMR/LLVM pair is recorded in the artifact by `wamrc` and nowhere in this platform, the device never reads it, and what is left is a build-host obligation — when the build reads the header WAMR would read at load, and how an artifact cache is keyed so a stale entry cannot outlive a version bump. It is concrete rather than hypothetical today: the `wamrc` anyone would build here comes from a tree nothing in this repository names, while the engine's is pinned at one `wamrx-sys` (§3.3, WAMR 2.4.3) — so this row owns making the two the *same* tree, and owns the wasm3 residue §3.3 records, where the crate that vendors the C source sits one level below the crate its callers name and `Cargo.lock` is the only thing pinning it.
...
- ~~**Memory budget**: heap sizing per target, and what a leaf does when a batch will not fit.~~ — **resolved** (eieio-x7g.2.7): §4.2 sizes it and §4.3 answers it. The allocator is `embedded-alloc`'s TLSF heap, one heap shared with the engine; the heap is the linker's remainder against a **192 KiB floor** derived for the v1 target's real 313 KiB of SRAM, which sizes an image for **two** block instances (see the amendments below, which have moved this number twice). `max_payload` is 4 096 (EXPR §9's `MAX_VALUE_BYTES` floor) and `max_batch` is 8. §4.1's decode bound is **resolved as the daemon's constant, defended against a measured stack**: `Value::decode_at` is a 160-byte frame on `riscv32imc-unknown-none-elf`, so host parity costs 15 KiB more stack than the floor would, against a 32 KiB reserve. Two measurements do the work and both are recorded in place: a batch's decoded footprint expands by between 1.19× and **22.1×**, so `max_payload` does not bound host memory and a leaf reserves before it decodes; and every golden block declares **17 pages** of linear memory today, three and a half times the whole chip, which a link flag fixes and which a leaf refuses at firmware build time until it is fixed.
  ...
  **Amended again** (eieio-7d8.41, eieio-x7g.2.29): the guest linear-memory row is **one** page per instance after all, and the two-page reading above was measuring an allocator rather than a block. `dlmalloc`'s wasm backend *will* take the ≈ 38 KiB `wasm-ld` leaves unused inside the declared page, but only when that span is at least one granule wide — and at its default 64 KiB granularity it never is, so the donation was declined once and, the flag being one-shot, for ever. SDK §4.1 configures the granularity to 4 096, swept rather than argued, and every scenario in §9's suite 1 now needs exactly one page. **The floor stays 192 KiB and what it buys goes back to two instances**, at 2 × 64 + 2 × 8 + 48 = 192 KiB exactly — so neither of the two hardware levers this section listed (`dram2_seg`, a bigger part) was spent, and the v1 target is unchanged. §4.2's instance count is now an *arithmetic* over the table's own rows (`V1_MAX_INSTANCES`) rather than a number, because it has been wrong in both directions inside one epic. The other half of eieio-x7g.2.29 is a refusal that did not exist: a service naming more instances than the floor carries is refused by `crates/leaf-gen` at firmware build time, listing them and naming the levers, in the same place and the same class as the per-instance page budget.
