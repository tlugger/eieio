...
- **Removing an OPTIONAL thing states an end state; removing an identified thing names something.** The two behave differently on absence and an editor MUST NOT blur them:
  - Clearing a `name`, a `[ui]` entry or any other OPTIONAL key **succeeds whether or not it was there**. The caller said "this is unset", and afterwards it is — the same reasoning ABI §7.2 gives for `state_del`. Erroring would make a caller track what it had already cleared, which is state the file already holds, and it is the Designer's exact case: a field cleared twice is not a mistake.
  - Removing a **block** by `id` fails when no such block exists, because the caller named an instance the file does not have and that is a different mistake — a graph the caller has misunderstood, not an end state it is asserting.

  All three OPTIONAL removals behave this way, `remove_prop` included: an unset property falls back to its manifest `default` (ABI §11.1), so "unset it" names a state that is already true. An editor SHOULD report *whether* it removed anything, so a caller can tell a person which happened — `eio service unset-prop` answers "already unset; nothing to do" and succeeds, because that is a report and not a failure.
...
- **The write is atomic, and that is the editor's property rather than a caller's.** The file is replaced, never truncated in place: a temporary file *in the same directory* as the target and a rename, because a rename across filesystems is a copy and not an atom. A service file is often open in another program, and a half-written one is worse than a failed write. `Document::write` is where this lives, so a second writer gets it by calling the crate — the alternative is two implementations of the same guarantee, which is the thing §9's one-editor rule exists to prevent. Note what is promised: a reader sees the old file or the new one, never a partial one.

- **Durability is part of the promise, and its limits are stated rather than implied.** `Document::write` syncs the temporary file *before* the rename, so no name can point at content the device has not got, and syncs the parent directory *after* it, so the entry the rename created is itself flushed. Reversing either order gives no guarantee: directory-first can expose garbage through a durable name, and never syncing the directory lets a crash undo a rename whose content was safe. This matters more than a lost edit because §3.8 makes a node's configuration these files and DAEMON §3 reads them at boot — a service file that survives a power cut as empty is a node coming up without a service it was told to run.
- **Two limits, both platform and neither hidden.** The directory half is Unix-only; on a non-Unix target the temporary's content is made durable and the directory entry is not. And a platform whose `fsync` returns before the device has flushed its own cache — macOS without `F_FULLFSYNC`, which this does not use — gives durability against an OS crash but not against sudden power loss. What is claimed everywhere is that the write survives the process and the operating system dying; surviving the power going out is what the platform's `fsync` is worth.
...
**Every mutating command is a §9 edit**: it reads the file, applies exactly what it was asked, and writes it through `Document::write`, which re-runs §7 stage 1 and replaces the file atomically (§9). A command that would leave the file invalid changes nothing and says which rule it broke. The CLI carries no write of its own — it is one of §9's two writers, not the place the guarantee lives.
