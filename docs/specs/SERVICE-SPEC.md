...
- **Removing an OPTIONAL thing states an end state; removing an identified thing names something.** The two behave differently on absence and an editor MUST NOT blur them:
  ...
  `remove_prop` presently errors on an unset property, which sits on the wrong side of this line: a property that is unset falls back to its manifest `default` (ABI §11.1), so "unset it" has a well-defined meaning that is already true. It is left as it is for now and tracked, rather than changed silently — see `eieio-m9s.10`.
