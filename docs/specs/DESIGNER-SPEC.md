...
**Who discharges the invalidation on a pull.** Nothing in the Designer installs a block today — no flow calls `POST /blocks/pull` — so the invalidation rule above has no trigger, and that is recorded here rather than left to be rediscovered: **an install flow MUST invalidate the pulled reference's cache entry as part of the same action**, re-fetching that reference from the node the pull was issued against and re-`PUT`ing it, before the palette or any of the three sites reads it again. `lib/api/manifests.ts`'s `supersedesOnPull` is that rule as a function — exact match on the whole reference, for the reason the paragraph below gives — and it is tested and deliberately uncalled. The obligation is this spec's; the function is what an install flow calls to discharge it.
