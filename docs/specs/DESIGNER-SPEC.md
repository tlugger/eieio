...
**Absence is not staleness, and a site answers it rather than fetching it** (eieio-m9s.45). Everything above is a rule about a manifest the Designer *holds* — whether its reference can have moved, and what to do if it has. A reference with **no cache entry at all** is a different question, and the staleness rule answers it only by accident: a revalidation with nothing to revalidate has nothing to say, and returns early. The case is ordinary rather than exotic, because a service file is the node's and not the Designer's: a reload picks up a hand-edited file, or one an agent wrote (SCOPE §4), naming a block this Designer has never browsed.

**No act site fetches the missing entry.** Two endpoints could be made to answer, and each answers a different question than the one being asked.

- `GET /blocks/available/{reference}` (DAEMON §9.8) answers what a **registry** offers, explicitly not what the node installed — which is why this section already marks every entry sourced from it *unverified* from the moment it is stored. Asking it "what are this block's ports and properties" gets back "what that tag offers right now", and the two differ precisely when it matters: the node pulled the tag before it moved. A stale manifest at an act site is the failure these three sites exist to prevent, so a site that manufactures one out of a registry is that same failure with an extra step — plus a cache write that makes it durable.
- `GET /blocks` cannot be asked at all, for the naming reason stated two rules below: a node keys its block cache by name and version (DAEMON §4) and renders every entry `name:version`, so a file naming `ghcr.io/tlugger/filter:1.2.0` has **no entry in that listing keyed by what it names**. Fetch-on-absence would therefore fail for exactly the references most likely to be absent — the registry-ful ones an operator pulled from a CLI or hand-wrote — and fail *silently*, back to the empty render it was added to prevent. A fix that works only where it was not needed is not one.

**And filling the cache stays where this section put it.** Browse, preview and install are three operator-initiated ways in, per node. A site that `PUT` an entry nobody asked for would be a fourth and an invisible one, growing the palette *unverified* registry-sourced entries on a double-click.

**So what a site does on absence follows from what the missing manifest was carrying there — refuse where it is the whole of what the site renders, degrade where it annotates something that is true without it.** That is why this is three rules rather than one verb.

- **The config modal MUST NOT open on a block with no cached manifest**, and MUST say which reference it lacks and how to add it. The modal *is* the manifest — ports, properties, the fields arriving at each input — so with none there is nothing to render. Opening anyway is the bug this rule closes: the modal announced "This block has no properties", which is not something it knows. It knows nothing *about* this block's properties, while the file's own `props` sit in the instance being neither shown nor editable, and an accept computed against an empty property list narrows an operator's edit to the name without saying so. The refusal is scoped to the Designer's own render and to nothing else — the node runs the service regardless, and the operator's way out is this section's own browse-and-preview.
- **A start MUST NOT be blocked.** The capability badge annotates a block the canvas draws either way, and `missingCapabilities` already yields no badge for a manifest it has not got — a block it cannot describe gets no warning rather than a false one. The node re-derives every block's capabilities from the WASM it is about to instantiate and refuses on its own (DAEMON §3); the badge previews that check and is never its authority. Refusing a start the node would accept would also hand the Designer a veto `eio services start` has not got, which §8 forbids.
- **A tap's `expr_failure` line MUST still render**, with the bare `prop` index eieio-m9s.14's resolver already falls back to. The name is an annotation on a real runtime failure, and hiding the failure behind a cache miss is strictly worse than showing the index.

**None of this adds a site.** The list is still the three above; what this settles is what those three do when the cache holds nothing for a reference.
