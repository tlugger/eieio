...
**Trust is whatever static configuration is worth**, and no more: whichever node answers on a configured address is taken to be that node. Authenticating the self-promotion step needs SCOPE §3.11, which is OPEN — this section depends on it rather than inventing a scheme, and says so instead of implying the bus is safe on an untrusted network.
