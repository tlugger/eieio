...
The daemon is the daemon-class node runtime (SCOPE §3.7): a single compiled Rust binary that establishes a node, executes services, and exposes the management API. This spec covers its internal architecture; the leaf runtime shares the starred (★) subsystems and is specified in LEAF-SPEC.
...
**Trap classification, and ABI §10's first obligation met by the engine.** A budget death **MUST** reach the host as a trap and not as a status code, and here it does so by construction: wasmtime unwinds an exhausted call as a `Trap`, so the classification below is a *reading* of the obligation rather than a second statement of it. Every arm discards the instance — ABI §5.1 offers no state to return to that is not "discard it" — but which death it was is what supervision (§8) and the operator's log need:
...
**The MQTT version is part of the bus contract, and today's is 3.1.1.** `crates/daemon` builds `rumqttc::MqttOptions`, which is MQTT 3.1.1; this was implicit until LEAF §8.3 measured that every `no_std` client a leaf can link is MQTT v5 only, and that `rumqttd` 0.20 binds v4 and v5 on separate listeners rather than negotiating one — so a bus is one version or it is not one bus. Recorded here because a bridge that is silent about its version is a bridge whose second implementation discovers the mismatch in the field. LEAF §8.3 states the requirement and names moving this bridge to `rumqttc`'s `v5` module — which ships unconditionally, with no feature and no new dependency — as the resolution; it has not been done, and until it is, a leaf cannot talk to this daemon.
