...
**Topics.** `eieio/<system>/<topic>`, where `<topic>` is the string a `publisher`/`subscriber` block carries as a property. The `eieio/` prefix keeps a shared broker usable by other tenants; `<system>` scopes a System against its neighbours on that broker, which is the same reason §10 keys state by node rather than trusting the network to be private. Both components follow ABI §11.1's name pattern, so a topic is never a path-traversal question and never needs escaping. A block's topic is *its* property and not a derived name: two instances may publish to one topic deliberately — §3.4 orders each publisher's own stream and makes no promise between them.
...
**The broker is an elected daemon-class node** (SCOPE §3.9), which the Designer MAY pin instead. The election mechanism itself — candidate set, lease, handover of in-flight publishes, and the case where no daemon-class node is reachable — is an expansion item and not specified here; at-most-once softens it, since a batch lost to a handover is permitted rather than a fault, but it does not answer who is broker next.
