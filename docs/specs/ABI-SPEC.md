...
- **It bounds admission, not growth, and a host that must bound growth as well does it in two places.** `memory.grow` is core WASM and a module MAY grow past its declared minimum; what bounds that is the module's declared *maximum* and the engine enforcing it. A host with an OS underneath it need bound nothing here — the daemon does not (DAEMON §4) — but a host on a fixed heap must, and the two cases below do not have one answer between them. LEAF §4.2 is the worked example and supplies the numbers; this is the rule both hosts share, because a leaf and a daemon differing on it would be §13's divergence in the one place a block author cannot see it coming.
    - **A module whose declared maximum exceeds the ceiling MUST be refused, in the same place and the same sense as one whose minimum does.** The module has stated an appetite the host cannot meet. Admitting it under a smaller cap is the bullet above one knob over: an instance allowed to grow less far than it declared fails at whatever allocation first crosses a line the guest was never told about, at whatever moment the traffic reaches it. So the ceiling is **one number judged against both ends of the declaration** — a minimum over it cannot be *supplied*, a maximum over it cannot be *honoured* — and `eio_manifest::validate_against` judges both in the walk that reads them, so a host cannot do the one and forget the other.
    - **A module that declares no maximum has stated nothing, and a host MAY bound its growth at the engine.** This is the ordinary case and not the exception: `wasm-ld` emits no maximum unless asked and SDK §5.2 deliberately does not ask, so every block built here declares one page and nothing on the right, which an engine reads as the WASM default of 65 536 pages. Capping such a module grants it nothing less than it declared, because it declared nothing; the host is choosing a maximum the module declined to choose, and the instance behaves exactly as it would have with that maximum written in its own memory section.

  **What a guest observes when growth is capped is not new ABI surface, and a host MUST NOT make it any.** `memory.grow` answers `-1`, which is core WASM's own result for a growth the engine will not perform — neither a trap nor a status code, and nothing this document defines. A guest allocator reads it as an allocation failure, and it reaches this ABI only where an allocation failure always did: `eio_alloc` returning 0, which §9.5 already makes `ERR_LIMIT`, counted like any other block-level error. §8's death kinds are closed and are untouched by this — in particular a host **MUST NOT** kill an instance for having grown, before or after the fact. That would be a fourth kind of death, and it would report a property of the host's budget as a fault of the guest, which is the same mistake as trapping a module the ceiling refuses.
...
10. **Declared linear memory** (§4.1), at either end, is the fourth host-configured number here and the only one a block cannot read. A host's per-instance page ceiling is **not** an instance-descriptor key and MUST NOT be published as one: the three above are numbers a live instance reacts to, and this one is answered before the instance exists — a module past the ceiling never runs, so there is no descriptor to carry it and no `ERR_LIMIT` for anyone to see. A block **MAY** declare more than one page and **MUST** assume nothing about any host's ceiling — including that `memory.grow` will succeed, which a host bounding growth may make it not (§4.1). Three things follow for a block author:
    ...
    Pinned by `34_memory_ceiling` and `35_memory_maximum` — the minimum a host must supply and the maximum it must honour — whose refusals are the loader's on every host (§13.1). There is deliberately no third scenario for a module declaring *no* maximum: bounding that one is a MAY, it is the engine's rather than the loader's, and what a guest sees when a host does it is `eio_alloc` returning 0, which `17_allocator_refuses` already covers.
