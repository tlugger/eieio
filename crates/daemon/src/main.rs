...
//!
//! Every module named above lives in this crate's lib target (`src/lib.rs`), not here — so
//! that a test outside this crate can reach them (DAEMON-SPEC §9's live OpenAPI document, in
//! particular) without spawning this binary. What stays in this file is argument parsing, the
//! `tracing` subscriber, and the two top-level verbs' bodies.
...
use eio_daemon::engine::Budgets;
use eio_daemon::{node, observe, run, run_node};
