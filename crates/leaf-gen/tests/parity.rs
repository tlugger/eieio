...
/// The golden-block page budget: every one of ABI §13.2's five declares 17 pages today.
///
...
/// budget is [`V1_MEMORY_PAGES`], one page; `wasm-ld` defaults the shadow stack to 1 MiB and
/// the fix belongs to SDK §5.2's link flags (eieio-x7g.2.21). Until that lands, a graph made
/// of golden blocks is baked against the budget the blocks actually have, and
/// [`the_v1_page_budget_refuses_every_golden_block`] is the test that keeps the gap visible.
const GOLDEN_BLOCK_PAGES: u64 = 17;
...
///
/// §4.2 predicted this from a measurement — "every golden block declares 17 pages", 1088 KiB,
/// three and a half times the whole v1 part — and made refusing it a MUST at firmware build
/// time. This test is where the prediction is checked rather than believed, and it is the
/// standing evidence for eieio-x7g.2.21: the fix is a `cargo eio build` link flag (SDK §5.2),
/// not a change to any block, so until it lands the generator refuses and says so.
#[test]
fn the_v1_page_budget_refuses_every_golden_block() {
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../examples/services/counter-transform.toml");
    let error = bake(&path, V1_MEMORY_PAGES).expect_err("17 pages is over a 1-page budget");
    ...
    assert_eq!(
        *declared, GOLDEN_BLOCK_PAGES,
        "the measurement LEAF §4.2 records"
    );
    assert_eq!(*budget, 1, "LEAF §4.2's v1 per-instance budget");
    let message = error.to_string();
