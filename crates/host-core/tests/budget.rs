...
//! both until [`Budgets`]. These tests are what say it now lives in the type instead.

use eio_expr::EvalLimits;
use eio_host_core::{Budgets, ErrorCode, Limits, Outbound};
...
fn a_decode_bound_below_the_expression_depth_cannot_be_built() {
    ...
    let budgets = Budgets::new(deep, 64);
...
fn the_relationship_holds_however_the_two_are_chosen() {
    ...
    // claim is that *no* pair of numbers produces a violating `Budgets`.
...
fn the_bound_a_budgets_carries_is_the_one_decode_applies() {
    ...
    assert_eq!(
        accept().decode(&deep, Budgets::DEFAULT),
...
fn a_batch_within_the_bound_still_decodes() {
    ...
    let batch = Outbound::accept(0, 8, 1, limits)
        .expect("port 0 of 1")
        .decode(&shallow, Budgets::DEFAULT)
