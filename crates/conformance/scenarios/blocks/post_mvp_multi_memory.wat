;; A block valid in every way except one: it uses multi-memory, which ABI §4.3 refuses.
;;
;; A second linear memory needs no instruction to be past the accepted set: declaring it is
;; already the proposal. The first memory is the ABI §4.1 export; this one is the flaw.
